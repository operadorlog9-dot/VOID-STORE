...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((o) =>
      o.setName('nota').setDescription('Nota de 1 a 5').setMinValue(1).setMaxValue(5).setRequired(true)
    )
    .addStringOption((o) =>
      o.setName('comentario').setDescription('Seu comentário').setRequired(true).setMaxLength(500)
    ),
  ...
  async execute(interaction) {
    ...
    if (!hasConfirmedPurchase(interaction.guild.id, interaction.user.id)) {
      return interaction.reply({
        content: 'A avaliação é liberada somente após uma compra confirmada neste servidor.',
        ephemeral: true
      });
    }
    ...
    if (!channel) {
      return interaction.reply({
        content: 'O canal #avaliacoes ainda não existe. Execute /montar-loja.',
        ephemeral: true
      });
    }
    ...
    await logAction(
      interaction.guild,
      `⭐ ${interaction.user.tag} publicou avaliação ${nota}/5.`
    );

    await interaction.reply({
      content: `✅ Avaliação publicada em ${channel}.`,
      ephemeral: true
    });
  }
};
